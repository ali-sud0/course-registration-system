...
def update_semester(
    ...
    if data.is_active is not None:
        if data.is_active:
            # Only one semester can be active; deactivate others
            db.query(Semester).filter(Semester.id != semester_id).update({Semester.is_active: False})
...
@router.get("/active/current", response_model=SemesterOut | None)
def get_active_semester_endpoint(db: Session = Depends(get_db)):
    from app.core.semester_helper import get_active_semester
    return get_active_semester(db)